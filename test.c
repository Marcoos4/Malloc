...
int main(void) {
    ...
    float count=1,i;  // Array di blocchi
    ...
    for(i=0; i<count; i++){
        ...
        float tem = end.tv_usec-sta.tv_usec;
        /* printf("Tempo passato prof: %2.f \n", tempo);
        printf("Tempo tutto: %2.f\n", tem); */
        float diff = tem-tempo;
        //printf("Differenza: %2.f\n", diff);

        if(diff>0)
            somma+=diff;
    }
    media = somma/count;
    printf("Media nostro: %2.f\n", media);
    return 0;
}